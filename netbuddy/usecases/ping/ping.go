...
func NewPing(params string, cfg settings.Config) (*Ping, error) {
	URL := params
	URL = Normalize(URL)
	u, err := url.Parse(URL)
	if err != nil {
...
func Hping(cfg settings.Config, args string) {
	// it should work at local mode
	if cPName != "local" {
		return
	}
	p, err := NewPing(args, cfg)
	if err != nil {
		println(err.Error())
	} else {
		p.Run()
