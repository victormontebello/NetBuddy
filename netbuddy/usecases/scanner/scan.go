...
var spin = spinner.New(spinner.CharSets[26], 220*time.Millisecond)
...
func NewScan(args string, cfg settings.Config) (Scan, error) {
	var (
		scan Scan
		err  error
	)
...
func ScanPorts(cfg settings.Config, args string) {
	scan, err := NewScan(args, cfg)
	if err != nil {
